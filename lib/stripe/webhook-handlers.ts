...
export async function handleSubscriptionCreated(event: Stripe.Event) {
  ...
  try {
    ...
    if (!userId) {
      console.warn(
        `User not found for customer: ${subscription_data.customer}, subscription: ${subscription_data.id}`,
      );
      console.warn(
        "This might be a race condition or webhook ordering issue. Subscription will be processed when user is available.",
      );
    ...
    await db.insert(subscription).values({
      ...
      canceled_at: subscription_data.canceled_at
        ? new Date(subscription_data.canceled_at * 1000)
        : null,
    });
      ...
      try {
        ...
        if (error.code === "23505") {
          ...
          if (requeriedSubscription.length === 0) {
            throw new Error(
              `Subscription still not found after duplicate error: ${subscription_data.id}`,
            );
          }
...
export async function handleInvoicePaymentFailed(event: Stripe.Event) {
  ...
  try {
    ...
    if (existingInvoice.length > 0) {
      await db.update(invoice).set(invoiceValues).where(eq(invoice.invoice_id, invoice_data.id));
    } else {
      await db.insert(invoice).values(invoiceValues);
    }
